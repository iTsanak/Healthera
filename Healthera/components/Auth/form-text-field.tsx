...
import { TextInput, TextInputProps, TouchableOpacity, useColorScheme, View, ViewProps } from "react-native";
...
const FormTextField = forwardRef<TextInput, FormTextFieldProps>(function FormTextField(
  ...
  return (
    <View style={style} {...rest} className={`${isLoading ? "opacity-50" : ""} flex-1`}>
      <View className="flex-1">
        <View className="mx-2 pb-2">
          <ThemedText className={titleClassName}>{title}</ThemedText>
        </View>
        <View
          className={`${isFocused ? "bg-accent-light/60 dark:bg-accent-dark/60" : "bg-secondary-light/20 dark:bg-secondary-dark/20"} h-14 flex-1 rounded-xl`}
        >
          <View className="mx-2 flex-1 flex-row items-center">
            <TextInput
              placeholderTextColor={theme === "dark" ? "#f1f1f1b0" : "#0f0f0f80"}
